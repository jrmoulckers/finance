...
import { prepareBankConnectionsForErasure } from '../_shared/bank-revocation-outbox.ts';
...
async function deleteAccountData(
  ...
  // Step 2b: transactionally move every live connection credential into the
  // server-only revocation outbox and sever its beneficiary references before
  // any account/household row can be removed. Provider work continues after
  // erasure without retaining a user/household FK (#4405).
  if (
    !(await prepareBankConnectionsForErasure(supabase, {
      ownerId: user.id,
      householdIds: soleHouseholdIds,
    }))
  ) {
    throw new Error('REVOCATION_HANDOFF_FAILED');
  }
