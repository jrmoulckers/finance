...
import { severBankRevocationIdentitiesForAccount } from '../_shared/bank-revocation-outbox.ts';
...
async function deleteAccountData(
  ...
  // Processor erasure is durably enqueued before ANY credential-bearing row is
  // deleted. The RPC also severs user/household/connection identifiers from
  // the outbox, so account deletion can continue through a provider outage
  // without leaving identity linkage behind.
  await severBankRevocationIdentitiesForAccount(supabase, {
    ownerId: user.id,
