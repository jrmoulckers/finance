...
import { enqueueAccountBankRevocations } from '../_shared/bank-revocation-queue.ts';
...
async function deleteAccountData(
  ...
  // Move every affected live/orphan provider credential into the single
  // server-only outbox before deleting any identity or connection row. This is
  // a database-transactional handoff: provider outages never block deletion,
  // but a failed durable enqueue stops before any credential can be lost.
  await enqueueAccountBankRevocations(supabase, {
    ownerId: user.id,
  ...
  if (error) {
    console.warn(
      JSON.stringify({
        ...
        function: 'account-delete',
        rpc: functionName,
        message: 'Best-effort erasure RPC failed',
      }),
    );
  }
}
  ...
  if (error) {
    console.warn(
      JSON.stringify({
        ...
        table,
        column,
        message: 'Best-effort pointer cleanup failed',
      }),
    );
  }
}
