...
Deno.test('account-delete durably hands off provider revocation before deleting rows', async () => {
  withEnv();
  const fake = createFakeSupabase();
  ...
  assertEquals(response.status, 204);
  const handoffIndex = fake.operations.indexOf('rpc:prepare_bank_connection_erasure');
  assert(handoffIndex >= 0);
  assert(handoffIndex < fake.operations.indexOf('delete:bank_connections'));
  assert(handoffIndex < fake.operations.indexOf('delete:users'));
  const handoff = fake.rpcCalls.find((call) => call.name === 'prepare_bank_connection_erasure');
  assertEquals(handoff?.args.p_owner_id, 'user-1');
  assertEquals(handoff?.args.p_household_ids, ['household-1']);
});

Deno.test(
  'account-delete stops before erasure if the durable revocation handoff fails',
  async () => {
    withEnv();
    const fake = createFakeSupabase({ failRevocationHandoff: true });
...
Deno.test('account-delete response exposes no durable revocation metadata', async () => {
  withEnv();
  const fake = createFakeSupabase();
...
function createFakeSupabase(opts: FakeSupabaseOptions = {}): FakeState & { client: FakeClient } {
  const state: FakeState = {
    ...
    failRevocationHandoff: opts.failRevocationHandoff === true,
  };
  const client = {
    auth: {
    ...
    rpc: (name: string, args: Record<string, unknown>) => {
      ...
      if (name === 'prepare_bank_connection_erasure' && state.failRevocationHandoff) {
  ...
  return {
    ...
    get failRevocationHandoff() {
      return state.failRevocationHandoff;
    },
    client,
  };
}
