...
Deno.test(
  'account-delete durably enqueues processor erasure before deleting identity rows',
  async () => {
    withEnv();
    const fake = createFakeSupabase();
    ...
    assertEquals(response.status, 204);
    const enqueueIndex = fake.operations.indexOf('rpc:enqueue_bank_connection_erasure');
    assert(enqueueIndex >= 0);
    assert(enqueueIndex < fake.operations.indexOf('rpc:destroy_user_encryption_keys'));
    assert(enqueueIndex < fake.operations.indexOf('delete:bank_connections'));
    assert(enqueueIndex < fake.operations.indexOf('delete:users'));
    assert(enqueueIndex < fake.operations.indexOf('auth.deleteUser:user-1'));

    const enqueue = fake.rpcCalls.find((call) => call.name === 'enqueue_bank_connection_erasure');
    assertEquals(enqueue?.args.p_owner_id, 'user-1');
    assertEquals(enqueue?.args.p_household_ids, ['household-1']);
  },
);

Deno.test('account-delete stops before erasure if durable handoff cannot commit', async () => {
  withEnv();
  const fake = createFakeSupabase({ erasureEnqueueError: true });
  ...
  assertEquals(fake.operations.includes('delete:bank_connections'), false);
  assertEquals(fake.operations.includes('delete:users'), false);
  assertEquals(fake.deletedAuthUser, null);
});

Deno.test('account-delete never returns durable outbox data', async () => {
  withEnv();
  const fake = createFakeSupabase();
  ...
  assertEquals(response.status, 204);
  assertEquals((await response.text()).includes('outbox'), false);
});
...
function createFakeSupabase(opts: FakeSupabaseOptions = {}): FakeState & { client: FakeClient } {
  const state: FakeState = {
    ...
    erasureEnqueueError: opts.erasureEnqueueError === true,
  };
  const client = {
    auth: {
    ...
    rpc: (name: string, args: Record<string, unknown>) => {
      ...
      if (name === 'enqueue_bank_connection_erasure') {
        return state.erasureEnqueueError
