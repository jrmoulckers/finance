...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dynamic Type** | Rotor entry text and per-point announcements use scalable text styles and are never truncated by VoiceOver. At accessibility sizes (SwiftUI `dynamicTypeSize.isAccessibilitySize`, i.e. **≥ AX1**) the visual chart hands off to the stacked data table — matching #2834's "switch to stacked rows at accessibility sizes" — but the descriptor's point set is unchanged, so the audio graph and rotors stay fully navigable. The always-available "View as table" toggle and the VoiceOver audio graph remain available at **every** size. Honor `largeContentViewer` for the rotor-invoking controls. |
