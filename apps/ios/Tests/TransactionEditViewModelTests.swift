...
final class TransactionEditViewModelTests: XCTestCase {
    ...
    @MainActor func testValidationFailsWithZeroAmount() async { let (vm,_,_) = makeViewModel(); await vm.loadData(); vm.amountText = "0"; let saved = await vm.save(); XCTAssertFalse(saved); XCTAssertTrue(vm.showingValidationError) }
    @MainActor func testValidationFailsWithEmptyPayee() async { let (vm,_,_) = makeViewModel(); await vm.loadData(); vm.payee = "   "; let saved = await vm.save(); XCTAssertFalse(saved); XCTAssertTrue(vm.showingValidationError) }
    @MainActor func testValidationFailsWithoutAccount() async { let (vm,_,_) = makeViewModel(); await vm.loadData(); vm.selectedAccountId = nil; let saved = await vm.save(); XCTAssertFalse(saved); XCTAssertTrue(vm.showingValidationError) }
    @MainActor func testSaveSucceedsWithValidData() async { let (vm,r,_) = makeViewModel(); await vm.loadData(); vm.payee = "Updated Payee"; let saved = await vm.save(); XCTAssertTrue(saved); XCTAssertEqual(r.updatedTransactions.count, 1); XCTAssertEqual(r.updatedTransactions.first?.id, SampleData.expenseTransaction.id); XCTAssertEqual(r.updatedTransactions.first?.payee, "Updated Payee") }
    @MainActor func testSavePreservesOriginalStatus() async { let (vm,r,_) = makeViewModel(transaction: SampleData.pendingTransaction); await vm.loadData(); vm.payee = "New"; let saved = await vm.save(); XCTAssertTrue(saved); XCTAssertEqual(r.updatedTransactions.first?.status, .pending) }
    @MainActor func testSaveFailureCapturesError() async { let r = StubTransactionRepository(); r.errorToThrow = TestError.simulated; let (vm,_,_) = makeViewModel(transactionRepo: r); await vm.loadData(); vm.payee = "X"; let saved = await vm.save(); XCTAssertFalse(saved); XCTAssertNotNil(vm.errorMessage); XCTAssertFalse(vm.isSaving) }
    @MainActor func testDeleteSucceeds() async { let (vm,r,_) = makeViewModel(); let deleted = await vm.delete(); XCTAssertTrue(deleted); XCTAssertEqual(r.deletedTransactionIds.count, 1); XCTAssertEqual(r.deletedTransactionIds.first, SampleData.expenseTransaction.id) }
    @MainActor func testDeleteFailureCapturesError() async { let r = StubTransactionRepository(); r.errorToThrow = TestError.simulated; let (vm,_,_) = makeViewModel(transactionRepo: r); let deleted = await vm.delete(); XCTAssertFalse(deleted); XCTAssertNotNil(vm.errorMessage); XCTAssertFalse(vm.isDeleting) }
