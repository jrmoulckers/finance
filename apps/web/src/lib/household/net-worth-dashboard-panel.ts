...
export function buildNetWorthDashboardPanelModel(params: {
  ...
  const latestSnapshot = [...params.snapshots]
    ...
    .at(-1);
  const goalRows = latestSnapshot
    ? compareNetWorthToSharedGoals(latestSnapshot, params.sharedGoals ?? [])
    : [];
  ...
  return {
    ...
    emptyState:
      params.rollup.assetCents === 0 && params.rollup.liabilityCents === 0
        ? 'No opted-in household net-worth accounts yet.'
        : null,
  };
}
