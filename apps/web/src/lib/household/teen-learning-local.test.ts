...
describe('buildTeenLearningRecordFromChild', () => {
  it('seeds practice balances from allowance balance and completed chores', () => {
    const record = buildTeenLearningRecordFromChild('household-1', {
      id: 'child-1',
      name: ' Taylor ',
      age: 14,
      balance: 22.5,
...
describe('upsertTeenLearningRecord', () => {
  it('replaces an existing record for the same child instead of duplicating it', () => {
    const first = buildTeenLearningRecordFromChild('household-1', {
      id: 'child-1',
      name: 'Taylor',
      age: 14,
      balance: 10,
    ...
    const next = buildTeenLearningRecordFromChild('household-1', {
      id: 'child-1',
      name: 'Taylor',
      age: 14,
      balance: 12,
...
describe('buildTeenLearningPayload', () => {
  it('filters local-first persistence to one household and excludes adult finance fields', () => {
    ...
    const otherRecord = { ...record, id: 'teen-learning:household-2:child-1', householdId: 'household-2' };
    ...
    expect(payload.privacyNotice).toContain('adult accounts, transactions, and net worth are excluded');
  });
});
