...
const transactions: SourceAccountTransaction[] = [
  { rowIndex: 0, date: '2024-01-15', payee: 'Coffee', amountCents: -500, sourceAccountName: 'Checking', sourceAccountId: 'chk' },
  { rowIndex: 1, date: '2024-01-16', payee: 'Pay', amountCents: 1000, sourceAccountName: 'Savings', sourceAccountId: 'sav' },
];
...
describe('account mapping review', () => {
  it('builds review rows with proposed matches and create-new suggestions', () => {
    ...
    expect(review.rows.find((row) => row.sourceName === 'Checking')?.selectedAccountId).toBe('acct-checking');
  ...
  it('applies reviewed overrides before commit planning', () => {
    ...
    const review = buildAccountMappingReview(plan, [], [
    ...
    expect(applied.routedTransactions.map((row) => row.targetAccountId)).toEqual(['acct-checking', 'acct-savings']);
