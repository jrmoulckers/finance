...
function thresholdFor(
  ...
  if (snapshot.marketSession === 'closed')
    return { freshMs: 0, delayedMs: policy.closedMarketGraceMs };
  if (snapshot.assetKind === 'crypto' || snapshot.marketSession === '24x7')
    return { freshMs: policy.cryptoFreshMs, delayedMs: policy.cryptoFreshMs * 2 };
  if (snapshot.assetKind === 'option')
    return { freshMs: policy.optionFreshMs, delayedMs: policy.openMarketDelayedMs };
...
export function evaluateQuoteFreshness(
  ...
  | EvaluatedQuote
  | { readonly freshness: 'missing'; readonly ageMs: 0; readonly staleReason: string } {
  if (!snapshot)
    return { freshness: 'missing', ageMs: 0, staleReason: 'No quote snapshot is available.' };
  if (snapshot.error)
    return {
      ...snapshot,
      freshness: 'failed',
      ageMs: ageMs(snapshot.asOf, now),
  ...
  if (age <= thresholds.delayedMs)
    return {
      ...snapshot,
      freshness: 'delayed',
      ageMs: age,
      staleReason: 'Quote is delayed but still usable.',
    };
  return {
    ...snapshot,
    freshness: 'stale',
    ageMs: age,
    staleReason: 'Quote exceeds freshness policy.',
  };
}
