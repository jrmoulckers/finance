...
export type P2PMatchType = 'reimbursement' | 'roommate-rent' | 'meal-split' | 'pass-through-transfer' | 'ambiguous';
...
function classify(transactions: readonly P2PTransaction[]): { type: P2PMatchType; confidence: number; reason: string } {
  const joined = transactions.map((transaction) => `${transaction.memo ?? ''} ${transaction.category ?? ''}`).join(' ').toLowerCase();
  if (/rent|utilities|roommate/.test(joined)) return { type: 'roommate-rent', confidence: 0.9, reason: 'Memo/category indicates recurring roommate housing reimbursement.' };
  if (/dinner|lunch|meal|restaurant|split/.test(joined)) return { type: 'meal-split', confidence: 0.82, reason: 'Memo/category indicates shared meal split.' };
  if (/transfer|move money|self/.test(joined)) return { type: 'pass-through-transfer', confidence: 0.8, reason: 'Memo indicates pass-through/self transfer.' };
  return { type: 'reimbursement', confidence: 0.7, reason: 'Opposite signed transactions net near zero.' };
}
...
export function matchP2PTransactions(input: P2PMatchInput): readonly P2PMatch[] {
  ...
  const overrides = new Map((input.overrides ?? []).map((override) => [overrideKey(override.transactionIds), override]));
  ...
  for (const expense of sorted) {
    ...
    const candidate = sorted.find((item) => {
      if (used.has(item.id) || item.id === expense.id || item.amountCents <= 0 || item.currency !== expense.currency) return false;
      if (daysApart(expense.date, item.date) > maxDaysApart) return false;
      const sameCounterparty = !expense.counterparty || !item.counterparty || expense.counterparty.toLowerCase() === item.counterparty.toLowerCase();
      return sameCounterparty && Math.abs(expense.amountCents + item.amountCents) <= Math.max(amountToleranceCents, Math.round(Math.abs(expense.amountCents) * 0.55));
    });
    ...
    matches.push({
      ...
      auditTrail: [detected.reason, ...(override ? [`Override ${override.state} at ${override.updatedAt}: ${override.reason}`] : [])],
    });
    used.add(expense.id);
