...
describe('market data freshness', () => {
  it('marks open equity quotes fresh, delayed, or stale', () => {
    ...
    expect(evaluateQuoteFreshness(quote({ asOf: '2026-01-02T14:50:00.000Z' }), now).freshness).toBe(
      'delayed',
    );
    expect(evaluateQuoteFreshness(quote({ asOf: '2026-01-02T14:00:00.000Z' }), now).freshness).toBe(
      'stale',
    );
  });

  it('uses 24x7 crypto freshness independently from market sessions', () => {
    const evaluated = evaluateQuoteFreshness(
      quote({
        symbol: 'BTC',
        assetKind: 'crypto',
        marketSession: '24x7',
        asOf: '2026-01-02T14:58:30.000Z',
      }),
      now,
    );
  ...
  it('keeps closed-market snapshots usable within grace window and surfaces failures', () => {
    expect(
      evaluateQuoteFreshness(
        quote({ marketSession: 'closed', asOf: '2026-01-01T20:59:00.000Z' }),
        now,
      ).freshness,
    ).toBe('delayed');
    expect(evaluateQuoteFreshness(quote({ error: 'provider timeout' }), now).freshness).toBe(
      'failed',
    );
  ...
  it('returns deterministic manual snapshots', async () => {
    ...
    await expect(
      provider.getSnapshots([
        { symbol: 'aapl', assetKind: 'equity' },
        { symbol: 'MSFT', assetKind: 'equity' },
      ]),
