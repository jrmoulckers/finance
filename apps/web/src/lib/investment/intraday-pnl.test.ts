...
function quote(
  ...
  return {
    symbol,
    assetKind: symbol === 'BTC' ? 'crypto' : 'equity',
    priceCents,
    currency: 'USD',
    asOf,
    source: 'manual',
    marketSession: symbol === 'BTC' ? '24x7' : 'open',
  };
}
...
describe('computeIntradayPnl', () => {
  it('computes cross-broker day, unrealized, realized, cash, and breakdown totals', () => {
    ...
    const report = computeIntradayPnl({
      positions,
      quotes: [quote('VTI', 110_00), quote('BTC', 41000_00)],
      realizedEvents: [
        {
          accountId: 'taxable',
          brokerage: 'A',
          symbol: 'AAPL',
          realizedPnlCents: 50_00,
          currency: 'USD',
        },
      ],
      cashMovements: [
