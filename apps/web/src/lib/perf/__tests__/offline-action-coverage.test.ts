...
describe('offline action coverage', () => {
  it('keeps supported account and transaction writes queueable', () => {
    ...
    expect(getOfflineActionCoverage('transaction', 'categorize')).toMatchObject({
  ...
  it('returns helpful disabled copy before unsupported writes submit', () => {
    ...
    expect(getDisabledOfflineActionCopy('receipt', 'upload')).toContain(
      'Receipt images are not queued',
    );
    expect(listUnsupportedOfflineWrites().map((rule) => rule.entity)).toEqual([
      'budget',
      'receipt',
      'settings',
    ]);
