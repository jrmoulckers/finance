...
describe('local wipe verification helpers', () => {
  it('verifies deleted and not-applicable local wipe areas', () => {
    const receipt = buildLocalWipeReceipt(
      'online',
      REQUIRED_LOCAL_WIPE_AREAS.map((area) =>
        localWipeOutcome(area, area === 'service-workers' ? 'not_applicable' : 'deleted'),
      ),
    );
  ...
  it('turns missing and failed instrumentation into failed receipt rows', () => {
    const receipt = buildLocalWipeReceipt(
      'online',
      [localWipeOutcome('opfs', 'failed', 'quota manager denied wipe')],
      ['opfs', 'indexeddb'],
    );
  ...
  it('does not claim server deletion for offline or demo mode', () => {
    ...
    expect(buildLocalWipeReceipt('demo', [], []).userCopy).toContain(
      'No production server deletion is claimed',
    );
