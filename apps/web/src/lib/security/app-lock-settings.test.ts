...
describe('app lock settings helpers', () => {
  ...
  it('locks on idle and resume policies without exposing sensitive metadata', () => {
    const settings = normalizeAppLockSettings({
      enabled: true,
      idleTimeoutMs: 60_000,
      lockOnResume: true,
    });
    const unlocked = reduceAppLockEvent(
      settings,
      DEFAULT_APP_LOCK_STATE,
      'unlock_success',
      1_000,
    ...
    expect(reduceAppLockEvent(settings, unlocked, 'idle_check', 61_000).state.reason).toBe(
      'idle_timeout',
    );
  ...
  it('explains disabled app lock separately from account login', () => {
    ...
    expect(
      reduceAppLockEvent(settings, DEFAULT_APP_LOCK_STATE, 'manual_lock', 1_000).audit,
    ).toMatchObject({
