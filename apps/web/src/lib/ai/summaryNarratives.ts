...
export function generateSummaryNarrative(
  ...
  const transactions = input.transactions.filter(
    (transaction) => transaction.date >= input.startDate && transaction.date <= input.endDate,
  );
  ...
  const comparisonSpending = (input.comparisonTransactions ?? [])
    .filter((transaction) => transaction.type === 'expense')
    .reduce((sum, transaction) => sum + Math.abs(transaction.amountCents), 0);
  const spendingChange =
    comparisonSpending > 0 ? (spending - comparisonSpending) / comparisonSpending : 0;
  ...
  const sources: NarrativeClaimSource[] = [
    {
      metric: 'incomeCents',
      value: income,
      sourceIds: incomes.map((transaction) => transaction.id),
    },
    {
      metric: 'spendingCents',
      value: spending,
      sourceIds: expenses.map((transaction) => transaction.id),
    },
  ];

  if (transactions.length === 0) {
    highlights.push(
      `No local transaction activity was recorded for ${input.startDate} through ${input.endDate}.`,
    );
    nextActions.push('Check back after new transactions sync or widen the date range.');
  } else {
    highlights.push(
      `${capitalize(input.period)} recap: income was ${money(income, input.locale)} and spending was ${money(spending, input.locale)}, for net cash flow of ${money(net, input.locale)}.`,
    );
    if (topCategory) {
      highlights.push(
        `${topCategory[0]} was the largest spending area at ${money(topCategory[1], input.locale)}.`,
      );
      sources.push({
        metric: `category:${topCategory[0]}`,
        value: topCategory[1],
        sourceIds: expenses
          .filter((transaction) => (transaction.category ?? 'Uncategorized') === topCategory[0])
          .map((transaction) => transaction.id),
      });
    }
    if (spendingChange >= 0.25)
      risks.push(
        `Spending was ${Math.round(spendingChange * 100)}% higher than the comparison period.`,
      );
    if (spendingChange <= -0.15)
      highlights.push(
        `Spending improved by ${Math.abs(Math.round(spendingChange * 100))}% versus the comparison period.`,
      );
  }
  ...
  for (const budget of input.budgets ?? []) {
    ...
    sources.push({
      metric: `budget:${budget.id}`,
      value: Math.round(percent * 100),
      sourceIds: [budget.id],
    });
    if (percent >= 1)
      risks.push(
        `${budget.name} is over budget; review recent transactions before adding new spend.`,
      );
    else if (percent >= 0.8)
      risks.push(`${budget.name} is close to its limit at ${Math.round(percent * 100)}% used.`);
  ...
  for (const goal of input.goals ?? []) {
    ...
    if (change > 0)
      highlights.push(
        `${goal.name} gained ${money(change, input.locale)} and is ${Math.round(progress * 100)}% funded.`,
      );
  }

  const unpaidBills = (input.bills ?? []).filter(
    (bill) => !bill.paid && bill.dueDate >= input.startDate && bill.dueDate <= input.endDate,
  );
  if (unpaidBills.length > 0) {
    risks.push(
      `${unpaidBills.length} bill(s) still need attention in this period, starting with ${unpaidBills[0].merchant}.`,
    );
    nextActions.push(`Review ${unpaidBills[0].merchant} before ${unpaidBills[0].dueDate}.`);
    sources.push({
      metric: 'unpaidBills',
      value: unpaidBills.length,
      sourceIds: unpaidBills.map((bill) => bill.id),
    });
  }
  ...
  if (unusual.length > 0) {
    risks.push(
      `Largest unusual transaction: ${unusual[0].merchant ?? 'Unknown merchant'} at ${money(Math.abs(unusual[0].amountCents), input.locale)}.`,
    );
    sources.push({
      metric: 'unusualTransaction',
      value: unusual[0].id,
      sourceIds: [unusual[0].id],
    });
  }
  ...
  if (nextActions.length === 0)
    nextActions.push(
      net >= 0
        ? 'Consider assigning part of the positive cash flow to a goal.'
        : 'Review flexible categories for one small adjustment next period.',
    );
...
function detectUnusualTransactions(expenses: readonly SummaryTransaction[]): SummaryTransaction[] {
  ...
  return expenses
    .filter((transaction) => Math.abs(transaction.amountCents) >= median * 3)
