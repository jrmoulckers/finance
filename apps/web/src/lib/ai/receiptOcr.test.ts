...
describe('receipt OCR extraction to transaction', () => {
  it('uses a local stub provider and maps receipt text to a transaction draft', async () => {
    ...
    expect(draft).toMatchObject({ merchant: 'Fresh Market', date: '2026-03-12', amountCents: 836, currency: 'USD', taxCents: 62, category: 'Groceries' });
  ...
  it('prefers prior user category choices over keyword rules', () => {
    const suggestion = suggestReceiptCategory({ merchant: 'Fresh Market', priorChoices: [{ merchant: 'fresh market', category: 'Household', tags: ['shared'] }] });
  ...
  it('requires review confirmation before save and persists accepted metadata', async () => {
    ...
    const saved = validateReceiptSave(draft, { confirmed: true, acceptedLineItems: [draft.lineItems[0]], category: 'Groceries' });
