...
const input: PeriodSummaryInput = {
  ...
  comparisonTransactions: [
    { id: 'old', date: '2026-02-22', amountCents: -50_000, type: 'expense', category: 'Mixed' },
  ],
  ...
  goals: [
    {
      ...
      name: 'Emergency fund',
      currentCents: 80_000,
      previousCurrentCents: 70_000,
      targetCents: 100_000,
    },
  ],
  bills: [
    { id: 'bill', merchant: 'Rent', dueDate: '2026-03-05', amountCents: 150_000, paid: false },
  ],
};
...
describe('summary narratives', () => {
  it('generates a source-backed weekly narrative with neutral highlights and risks', () => {
    ...
    expect(
      summary.sources.find((source) => source.metric === 'spendingCents')?.sourceIds,
    ).toContain('repair');
  });

  it('handles empty periods with a data-needed action', () => {
    const summary = generateSummaryNarrative(
      { period: 'monthly', startDate: '2026-04-01', endDate: '2026-04-30', transactions: [] },
  ...
  it('reports positive spending changes and localization-ready currency formatting', () => {
    const summary = generateSummaryNarrative({
      ...input,
      comparisonTransactions: [
        { id: 'old', date: '2026-02-01', amountCents: -200_000, type: 'expense' },
      ],
      locale: 'en-US',
    });
