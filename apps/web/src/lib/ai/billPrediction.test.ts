...
import {
  applyBillCandidateDecision,
  classifyCadence,
  detectBillCandidates,
...
describe('bill prediction and confirmation', () => {
  ...
  it('supports confirm, edit, ignore, and merge candidate flows', () => {
    ...
    const confirmed = applyBillCandidateDecision(candidates, {
      candidateId: rent?.id ?? '',
      action: 'confirm',
    });
    expect(confirmed.find((candidate) => candidate.id === rent?.id)?.status).toBe('confirmed');
    const edited = applyBillCandidateDecision(confirmed, {
      candidateId: rent?.id ?? '',
      action: 'edit',
      changes: { nextDueDate: '2026-04-01' },
    });
    expect(edited.find((candidate) => candidate.id === rent?.id)?.nextDueDate).toBe('2026-04-01');
    const ignored = applyBillCandidateDecision(edited, {
      candidateId: rent?.id ?? '',
      action: 'ignore',
    });
  ...
  it('dedupes bill-due and projected-low-balance notifications', () => {
    const [candidate] = applyBillCandidateDecision(detectBillCandidates(tx), {
      candidateId: 'bill-city rent',
      action: 'edit',
      changes: { nextDueDate: '2026-03-20' },
    });
    const notifications = generateBillNotifications([candidate], {
      today: '2026-03-18',
      confidenceThreshold: 0.5,
      leadDays: 7,
      projectedBalanceCents: 151_000,
      lowBalanceThresholdCents: 1_000,
    });
    expect(notifications.map((item) => item.type)).toEqual(['bill_due', 'projected_low_balance']);
    expect(
      generateBillNotifications([candidate], {
        today: '2026-03-18',
        confidenceThreshold: 0.5,
        leadDays: 7,
        existingDeduplicationKeys: notifications.map((item) => item.deduplicationKey),
      }),
    ).toHaveLength(0);
  });
});
