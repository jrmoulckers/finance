...
import {
  prioritizeAndBundleNotifications,
  scoreNotification,
  serializeNotificationBundles,
  type SmartNotification,
} from './notificationPrioritization';
...
const notifications: readonly SmartNotification[] = [
  ...
  },
  {
    ...
    severity: 'warning',
    title: 'Dining pace',
    message: 'Dining is ahead',
    createdAt: '2026-03-15T09:00:00.000Z',
    status: 'unread',
    entityType: 'budget',
    entityId: 'dining',
    financialImpactCents: 12_000,
  },
  {
    id: 'budget2',
    type: 'budget_threshold',
    severity: 'info',
    title: 'Dining check-in',
    message: 'Review dining',
    createdAt: '2026-03-15T10:00:00.000Z',
    status: 'unread',
    entityType: 'budget',
    entityId: 'dining',
    financialImpactCents: 2_000,
  },
  {
    id: 'dismissed',
    type: 'goal_nudge',
...
describe('smart notification prioritization', () => {
  it('scores by severity, impact, due proximity, entity type, and dismissal history', () => {
    ...
    const dismissed = scoreNotification(notifications[3], {
      now: '2026-03-15T12:00:00.000Z',
      history: { dismissedEntityIds: ['vacation'] },
    });
  ...
  it('keeps critical alerts unbundled and bundles related low or medium items', () => {
    const bundles = prioritizeAndBundleNotifications(notifications, {
      now: '2026-03-15T12:00:00.000Z',
    });
    ...
    expect(budgetBundle?.children.map((child) => child.notification.id)).toEqual([
      'budget1',
      'budget2',
    ]);
  });
  ...
  it('persists bundle history as child notification ids', () => {
    const bundles = prioritizeAndBundleNotifications(notifications.slice(1, 3), {
      now: '2026-03-15T12:00:00.000Z',
    });
