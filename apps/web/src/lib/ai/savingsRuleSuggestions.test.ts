...
import {
  applySavingsSuggestionDecision,
  detectPaydayCadence,
  estimateRoundUpSavings,
  suggestSavingsRules,
  type SavingsTransaction,
} from './savingsRuleSuggestions';
...
describe('automated savings-rule suggestions', () => {
  it('detects payday cadence and suggests user-approved rules from surplus', () => {
    expect(
      detectPaydayCadence(transactions.filter((transaction) => transaction.type === 'income')),
    ).toBe('biweekly');
    const suggestions = suggestSavingsRules(
      transactions,
      [
        {
          id: 'goal',
      ...
      ],
      {
        ...
        minimumSafeBalanceCents: 50_000,
      },
    );
  ...
  it('estimates round-up savings from observed purchases', () => {
    const estimate = estimateRoundUpSavings(
  ...
  it('suppresses suggestions when balance and upcoming bills imply risk', () => {
    const suggestions = suggestSavingsRules(transactions, [], {
      startDate: '2026-03-01',
      endDate: '2026-03-31',
      currentBalanceCents: 40_000,
      minimumSafeBalanceCents: 20_000,
      upcomingBills: [{ id: 'rent', merchant: 'Rent', dueDate: '2026-03-20', amountCents: 30_000 }],
    });
    expect(suggestions).toEqual([]);
  });

  it('requires explicit approval or dismissal and can link a chosen goal', () => {
    const [suggestion] = suggestSavingsRules(transactions, [], {
      startDate: '2026-03-01',
      endDate: '2026-03-31',
      currentBalanceCents: 500_000,
    });
    expect(
      applySavingsSuggestionDecision([suggestion], {
        suggestionId: suggestion.id,
        action: 'approve',
        targetGoalId: 'vacation',
      })[0],
    ).toMatchObject({ status: 'approved', targetGoalId: 'vacation' });
    expect(
      applySavingsSuggestionDecision([suggestion], {
        suggestionId: suggestion.id,
        action: 'dismiss',
