...
const KEYWORD_CATEGORIES: readonly { readonly category: string; readonly keywords: readonly string[]; readonly tags?: readonly string[] }[] = [
...
export function mapReceiptOcrToDraft(
  ...
  const lines = ocr.rawText.split(/\r?\n/u).map((line) => line.trim()).filter(Boolean);
  const merchant = extractMerchant(lines) || 'Unknown merchant';
  const date = extractDate(ocr.rawText) ?? options.defaultDate ?? new Date().toISOString().slice(0, 10);
  ...
  const total = extractLabelAmount(ocr.rawText, ['total', 'amount due', 'paid']) ?? Math.max(0, ...extractMoneyValues(ocr.rawText));
  const lineItems = extractLineItems(lines, total, taxCents, tipCents);
  const suggestion = suggestReceiptCategory({ merchant, lineItems, priorChoices: options.priorChoices });
  const duplicateWarning = findReceiptDuplicate({ merchant, date, amountCents: total }, options.recentTransactions ?? []);
...
export function suggestReceiptCategory(input: {
  ...
  const searchable = `${input.merchant} ${(input.lineItems ?? []).map((item) => item.description).join(' ')}`.toLowerCase();
  const match = KEYWORD_CATEGORIES.find((entry) => entry.keywords.some((keyword) => searchable.includes(keyword)));
  return { category: match?.category ?? 'Uncategorized', tags: match?.tags ?? [], confidence: match ? 0.72 : 0.25 };
}
...
export function findReceiptDuplicate(
  ...
  for (const transaction of recentTransactions) {
    ...
    const merchantMatches = normalize(transaction.merchant) === draftMerchant || normalize(transaction.merchant).includes(draftMerchant) || draftMerchant.includes(normalize(transaction.merchant));
    if (!merchantMatches || days > 3 || amountDelta > 100) continue;
    const score = 1 - Math.min(0.6, days * 0.15) - Math.min(0.3, amountDelta / Math.max(1, draft.amountCents));
    if (!best || score > best.score) {
      best = { transactionId: transaction.id, reason: 'Similar merchant, date, and amount already exist', score: Number(score.toFixed(2)) };
    }
  }
  return best;
}

export function validateReceiptSave(draft: ReceiptTransactionDraft, decision: ReceiptReviewDecision): ReceiptTransactionDraft {
  if (!decision.confirmed) throw new Error('Receipt transaction must be reviewed and confirmed before saving.');
  ...
  if (!merchant || merchant === 'Unknown merchant') throw new Error('Merchant is required before saving.');
  if (!/^\d{4}-\d{2}-\d{2}$/u.test(date)) throw new Error('A valid transaction date is required before saving.');
  if (!Number.isFinite(amountCents) || amountCents <= 0) throw new Error('A positive transaction amount is required before saving.');
...
function extractMerchant(lines: readonly string[]): string | undefined {
  return lines.find((line) => !/\d/u.test(line) && line.length >= 2)?.replace(/receipt|invoice/giu, '').trim();
}
...
function extractMoneyValues(text: string): number[] {
  return [...text.matchAll(/(?:[$ۣ]\s*)?(\d{1,5}(?:,\d{3})*\.\d{2})\b/gu)].map((match) => Math.round(Number(match[1].replaceAll(',', '')) * 100));
}
...
function extractLabelAmount(text: string, labels: readonly string[]): number | undefined {
  ...
  const match = text.match(new RegExp(`(?:${escaped})\\s*[:#-]?\\s*(?:[$ۣ]\\s*)?(\\d{1,5}(?:,\\d{3})*\\.\\d{2})`, 'iu'));
  return match ? Math.round(Number(match[1].replaceAll(',', '')) * 100) : undefined;
}

function extractLineItems(lines: readonly string[], total: number, tax?: number, tip?: number): ReceiptLineItem[] {
