...
describe('subscription detection', () => {
  it('normalizes merchants and detects monthly, annual, and trial conversion candidates', () => {
    ...
    expect(candidates.find((candidate) => candidate.merchant === 'Streamflix')).toMatchObject({
      cadence: 'monthly',
      annualizedCostCents: 15_600,
    });
    expect(candidates.find((candidate) => candidate.merchant === 'Cloud Backup')).toMatchObject({
      cadence: 'annual',
      annualizedCostCents: 12_000,
    });
    expect(candidates.find((candidate) => candidate.merchant === 'News')).toMatchObject({
      cadence: 'trial_conversion',
    });
  });
  ...
  it('flags material price changes with annual impact', () => {
    ...
    expect(changes[0]).toMatchObject({
      priorAmountCents: 1_000,
      newAmountCents: 1_300,
      deltaCents: 300,
      percentDelta: 30,
      annualImpactCents: 3_600,
    });
  });
  ...
  it('supports confirm, rename, merge, dismiss, cancel, and price-change acknowledgement', () => {
    ...
    const renamed = applySubscriptionDecision(candidates, {
      candidateId: 'sub-streamflix',
      action: 'rename',
      name: 'StreamFlix Family',
    });
    expect(renamed.find((candidate) => candidate.id === 'sub-streamflix')?.merchant).toBe(
      'StreamFlix Family',
    );
    const acknowledged = applySubscriptionDecision(renamed, {
      candidateId: 'sub-streamflix',
      action: 'acknowledge-price-change',
      effectiveDate: '2026-03-01',
    });
    expect(
      acknowledged.find((candidate) => candidate.id === 'sub-streamflix')?.priceChanges[0]
        .acknowledged,
    ).toBe(true);
    expect(
      applySubscriptionDecision(acknowledged, {
        candidateId: 'sub-streamflix',
        action: 'dismiss',
      }).find((candidate) => candidate.id === 'sub-streamflix')?.status,
    ).toBe('dismissed');
    expect(
      applySubscriptionDecision(acknowledged, {
        candidateId: 'sub-streamflix',
