...
describe('delivery controls', () => {
  it('validates quiet hour time ranges', () => {
    expect(validateQuietHours({ enabled: true, startTime: '22:00', endTime: '07:00' }).valid).toBe(
      true,
    );
    expect(validateQuietHours({ enabled: true, startTime: '24:00', endTime: '07:00' }).valid).toBe(
      false,
    );
    expect(validateQuietHours({ enabled: true, startTime: '07:00', endTime: '07:00' }).valid).toBe(
      false,
    );
  });
  ...
  it('fills missing channel preferences from defaults', () => {
    ...
    expect(
      normalized.channelPreferences.some(
        (preference) => preference.alertType === 'spending_digest',
      ),
    ).toBe(true);
    expect(
      normalized.channelPreferences.some(
        (preference) => preference.alertType === 'subscription_price_change',
      ),
    ).toBe(true);
  });

  it('updates channel controls for one alert type', () => {
    const updated = setAlertChannels(DEFAULT_NOTIFICATION_PREFERENCES, 'spending_digest', [
      'in_app',
      'email',
    ]);

    expect(
      updated.channelPreferences.find((preference) => preference.alertType === 'spending_digest')
        ?.channels,
  ...
  it('explains quiet-hours and channel suppression decisions', () => {
    ...
    expect(decisions.find((decision) => decision.channel === 'email')?.reason).toBe(
      'channel_disabled',
    );
  });
