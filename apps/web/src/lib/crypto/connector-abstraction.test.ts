...
describe('crypto connector abstraction', () => {
  it('serves deterministic manual balances and transactions', async () => {
    ...
    await expect(provider.getBalances('wallet')).resolves.toEqual([
      expect.objectContaining({ asset: 'ETH' }),
    ]);
    await expect(provider.getTransactions('wallet')).resolves.toEqual([
        ...
        [
          {
            ...
            label: 'A',
            health: 'healthy',
            lastSyncAt: '2025-12-31T00:00:00.000Z',
          },
        ],
        1000,
