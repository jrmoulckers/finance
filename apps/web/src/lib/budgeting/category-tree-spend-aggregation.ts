...
function ancestorIds(categoryId: string, categoryById: ReadonlyMap<string, CategoryTreeSpendCategory>): readonly string[] {
  const ids: string[] = [];
...
function transactionAmounts(transaction: CategoryTreeSpendTransaction): readonly CategoryTreeSpendSplit[] {
  if (transaction.splits && transaction.splits.length > 0) return transaction.splits;
  return transaction.categoryId ? [{ categoryId: transaction.categoryId, amountCents: transaction.amountCents }] : [];
}
...
export function aggregateCategoryTreeMonthlySpend(input: {
  ...
  const months = Array.from({ length: lookbackMonths }, (_, index) => addMonths(input.asOfMonth, index - lookbackMonths + 1));
  const monthSet = new Set(months);
  const categoryById = new Map(input.categories.map((category) => [category.id, category]));
  const totals = new Map<string, number>();

  for (const transaction of input.transactions) {
    if (transaction.deleted || transaction.kind === 'income' || transaction.kind === 'transfer') continue;
  ...
  const monthlySpend = [...totals.entries()]
    ...
    .sort((left, right) => left.categoryId.localeCompare(right.categoryId) || left.monthKey.localeCompare(right.monthKey));
