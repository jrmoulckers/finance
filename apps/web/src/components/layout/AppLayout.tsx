...
export const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  const s = useCallback(() => onNavigate('/settings'), [onNavigate]);
  return (
    <div className="app-layout">
      <a href="#main-content" className="skip-link">
        Skip to main content
      </a>
      <SidebarNavigation activePath={activePath} onNavigate={onNavigate} />
      <header className="app-header" aria-label="App header">
        <h1 className="app-header__title">{pageTitle}</h1>
        <div className="app-header__actions">
          <button type="button" className="icon-button" aria-label="Settings" onClick={s}>
