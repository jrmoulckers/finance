...
export const ThirdPartyPermissionReview: React.FC = () => {
  ...
  const [educationAcknowledged, setEducationAcknowledged] = useState(() =>
    hasAcknowledgedThirdPartyEducation(),
  );
    ...
    <section aria-label="Third-party connections" className="page-section">
      <div className="settings-group">
        ...
        <p className="privacy-settings__info">
          Review external access before sharing data. Check the domain, never share passwords or
          one-time codes, and disconnect stale or suspicious permissions.
        </p>
        ...
        {!educationAcknowledged && (
          <button
            ...
            onClick={() => void acknowledgeEducation()}
          >
            <span className="settings-item__label">
              Acknowledge scam-resistant sharing guidance
            </span>
        ...
        {connections.length === 0 ? (
          <p className="privacy-settings__info">
            No third-party apps, banks, sync providers, or export recipients currently have saved
            access.
          </p>
        ) : (
            ...
            <div className="settings-item settings-item--static" key={connection.id}>
              <div>
                ...
                <p className="privacy-settings__category-description">
                  {connection.type} · {connection.scopes.join(', ') || 'No scopes recorded'} ·
