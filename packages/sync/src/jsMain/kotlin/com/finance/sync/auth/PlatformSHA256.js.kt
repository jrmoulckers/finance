 ...
 *
 * Uses Web Crypto API for browser environments (synchronous wrappers
 * around the globalThis.crypto API). Falls back to Node.js crypto
 * module when Web Crypto is unavailable.
 */
actual object PlatformSHA256 {
    @Suppress("UnsafeCastFromDynamic")
    actual fun sha256(input: ByteArray): ByteArray {
        // Use a pure-Kotlin SHA-256 implementation for synchronous cross-env compatibility.
        // Web Crypto's digest is async-only, and Node's require('crypto') isn't in browsers.
        return Sha256.digest(input)
    }

    @Suppress("UnsafeCastFromDynamic")
    actual fun randomBytes(size: Int): ByteArray {
        // crypto.getRandomValues works in both browser and Node 19+
        val array = js("new Uint8Array(size)")
        js("(globalThis.crypto || require('crypto').webcrypto).getRandomValues(array)")
        return ByteArray(size) { i: Int -> (array[i] as Number).toByte() }
    }
}

/**
 * Pure-Kotlin SHA-256 implementation.
 * Avoids async Web Crypto and Node.js-only require('crypto').
 */
